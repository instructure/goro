...
const PATH_FORMATTER = path.resolve(__dirname, '..', 'ext', 'rspec', 'jojo_formatter.rb');
const { validate, compose } = require('./JojoUtils');
const { shape, string, bool, number, func } = require('react-schema').PropTypes;
const ExecutionThread = require('./JojoExecutionThread');

function JojoExecutor(dependencies) {
  validate(shape({
    console: shape({
  ...
  function runRSpec() {
    if (isBusy()) {
      console.log('Delaying execution; RSpec is already running.');
